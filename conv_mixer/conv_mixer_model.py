...
def testOneForwardPass():
    ...
    num_blocks = 4 # 2, 4, 8
    conv_kernel_shape = (7, 13) #
    dimPosEmb = 50 # 32, 64, 128
    conv_nChan = 2 # 1, 2, 4, 8
    encoder_n_harmonic_functions = 64 # -1, 32, 64

    conv_padding = (
        (conv_kernel_shape[0] - 1) / 2,
        (conv_kernel_shape[1] - 1) / 2,
    )
    # assert padding is integer
    assert conv_padding[0] == int(conv_padding[0]) and conv_padding[1] == int(conv_padding[1])
    conv_padding = (int(conv_padding[0]), int(conv_padding[1]))
