        ...
        for cnt, batch in enumerate(data_loader):
            ...
            if args.loss_type == 'mpjpe':
                ...
                sequences_gt = batch[:, args.input_n:args.input_n +
                                                     args.output_n, dim_used].view(-1, args.output_n, len(dim_used))
            elif args.loss_type == 'angle':
                sequences_train = batch[:, 0:args.input_n, dim_used].view(
                    -1, args.input_n, len(dim_used))
    ...
    for action in actions:
        ...
        for cnt, batch in enumerate(test_loader):
            ...
            auc_pck_batch = auc_pck_metric(
                sequences_predict.view(-1, args.output_n, len(dim_used) // 3, 3) ,#/ 1000,
                sequences_gt.view(-1, args.output_n, len(dim_used) // 3, 3) ,#/ 1000
            )
            ...
            if save_results and cnt in sequences_to_save:
                ...
                batch_predicted = batch[10, args.input_n:args.input_n + args.output_n, :].clone()
                ...
                visualize_batch_ais(
                    ...
                    batch_gt=batch[10, args.input_n:args.input_n + args.output_n, :].cpu(),
                    batch_train=batch[10, 0:args.input_n, :].cpu()
                )
